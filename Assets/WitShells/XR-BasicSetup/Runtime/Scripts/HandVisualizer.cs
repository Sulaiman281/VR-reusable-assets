...
namespace WitShells.XR
{
    ...
    public class HandVisualizer : MonoBehaviour
    {
        ...
        // Public tracked state you can query from other scripts
        public bool leftHandTracked { get; private set; }
        public bool rightHandTracked { get; private set; }
        public bool anyHandTracked => leftHandTracked || rightHandTracked;
        ...
        protected void OnEnable()
        {
            ...
            leftHandTracked = m_Subsystem.leftHand.isTracked;
            rightHandTracked = m_Subsystem.rightHand.isTracked;

            UpdateRenderingVisibility(m_LeftHandGameObjects, leftHandTracked);
            UpdateRenderingVisibility(m_RightHandGameObjects, rightHandTracked);
        }
        ...
        void OnUpdatedHands(XRHandSubsystem subsystem, XRHandSubsystem.UpdateSuccessFlags updateSuccessFlags, XRHandSubsystem.UpdateType updateType)
        {
            ...
            // Update public flags
            this.leftHandTracked = leftHandTracked;
            this.rightHandTracked = rightHandTracked;
        ...
        class HandGameObjects
        {
                ...
                GameObject velocityPrefab)
            {
                    ...
                    Transform drawJointsParent)
                {
                    ...
                    GameObject jointGO;
                    if (debugDrawPrefab != null)
                    {
                        jointGO = Instantiate(debugDrawPrefab);
                    }
                    else
                    {
                        jointGO = new GameObject(jointId.ToString());
                        jointGO.AddComponent<LineRenderer>();
                        Debug.LogWarning($"HandVisualizer: m_DebugDrawPrefab not assigned. Creating fallback joint marker for {jointId}.");
                    }

                    jointGO.transform.parent = drawJointsParent;
                    jointGO.name = jointId.ToString();
                    m_DrawJoints[jointIndex] = jointGO;

                    GameObject velocityGO;
                    if (velocityPrefab != null)
                    {
                        velocityGO = Instantiate(velocityPrefab);
                    }
                    else
                    {
                        velocityGO = new GameObject(jointId + "_Velocity");
                        Debug.LogWarning($"HandVisualizer: m_VelocityPrefab not assigned. Creating fallback velocity container for {jointId}.");
                    }
                    velocityGO.transform.parent = jointDrivenTransform;
                    m_VelocityParents[jointIndex] = velocityGO;

                    var line = m_DrawJoints[jointIndex].GetComponent<LineRenderer>();
                    if (line == null)
                        line = m_DrawJoints[jointIndex].AddComponent<LineRenderer>();
            ...
            public void ToggleDebugDrawJoints(bool debugDrawJoints)
            {
                ...
                {
                    var go = m_DrawJoints[jointIndex];
                    if (go != null)
                        ToggleRenderers<MeshRenderer>(debugDrawJoints, go.transform);
                    if (m_Lines[jointIndex] != null)
            ...
            public void SetVelocityType(VelocityType velocityType)
            {
                ...
                {
                    var vel = m_VelocityParents[jointIndex];
                    if (vel != null)
                        ToggleRenderers<LineRenderer>(velocityType != VelocityType.None, vel.transform);
                }
            }
                ...
                bool cacheParentPose = true)
            {
                ...
                var jv = m_JointVisualizers[jointIndex];
                if (jv != null)
                    jv.NotifyTrackingState(joint.trackingState);
                ...
                {
                    if (m_DrawJoints[parentIndex] != null && m_DrawJoints[jointIndex] != null && m_Lines[jointIndex] != null)
                    {
                ...
                if (velocityType != VelocityType.None && m_VelocityParents[jointIndex] != null && m_VelocityParents[jointIndex].TryGetComponent<LineRenderer>(out var renderer))
                {
